# Vaatimusmäärittely  
## Sovelluksen tarkoitus  
Muistikorttisovellus, jossa käyttäjä voi tehdä muistikortteja (_flashcard_), jossa toisella puolella on jokin tehtävä ja sitten kääntöpuolella sen oikea vastaus. Näitä sovelluksessa tehtyjä kortteja voi käyttää esimerkiksi kielen sanaston tai vaikka jonkin alan käsitteiden opetteluun/kertaukseen.  

## Käyttäjäroolit   
Peruskäyttäjä.  

## Perustoiminnallisuus  
- Käyttäjä voi tehdä uuden tunnuksen.
- Käyttäjä voi kirjautua sisään tunnuksellaan.
- Jos kirjautuminen epäonnistuu, sivu ilmoittaa siitä. Muuten käyttäjälle ilmoitetaan kirjautumisen onnistuneen ja ohjataan käyttäjä etusivulle.
### Sisäänkirjautumisen jälkeen  
- Käyttäjä voi luoda omia muistikortteja.
- Käyttäjä selata tekemiään aiempia kortteja.
- Käyttäjä voi tarkastella yksittäistä korttia.
- Käyttäjä voi valita korttinäkymässä korttia klikkaamalla, milloin haluaa nähdä oikean vastauksen.
- Käyttäjä voi muokata luomiaan kortteja.
- Käyttäjä voi poistaa luomansa kortin.
- Käyttäjä voi muodostaa omia tekemistään korteista koostuvia pakkoja esim. tietyn aiheen mukaan.
- Käyttäjä voi filtteröidä kortteja pakan mukaan.
- Käyttäjä voi kirjautua ulos sovelluksesta.

## Jatkokehitysideat  
- Kortin pakan vaihtaminen.
- Pakan poistaminen.
- Käyttäjä voi valita opiskellessa, menikö vastaus oikein vai tarvitseeko vielä kertausta, ja jakaa kortit luokkiin niiden vaikeustason mukaan. Näitä luokkia voisi olla vaikka 3, ja vaikeampia kortteja kysytään enemmän, helppoja taas harvemmin.
- Aikavälitoiminto, jossa voi valita korttien toiston välin.
- Käyttäjä voi nähdä tilastoja sovelluksesta, esim. kuinka monta korttia on luonut ja kuinka monta korttia on mennyt oikein siihen mennessä.
- Julkiset pakat, joita voi jakaa sovelluksen muiden käyttäjien kanssa.
- Julkiset käyttäjäprofiilit, niiden muokkaaminen ja poisto.